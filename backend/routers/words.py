...
async def list_words(pagination: Params = Depends(), session: AsyncSession = Depends(get_session)):
    words = await Words(session).get_list(pagination)
    return words


@router.get('/{word_id}', status_code=status.HTTP_200_OK, response_model=WordPublic)
async def list_words(word_id: int, session: AsyncSession = Depends(get_session)):
